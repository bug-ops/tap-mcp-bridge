...
- **Breaking (security):** `tap-mcp-server`'s `Config::validate_signing_key` now rejects `TAP_SIGNING_KEY` values whose 32 decoded bytes are all identical (covers the all-zero `0000...0000`, all-ones `ffff...ffff`, and any all-byte-equal placeholder pattern). The previous validator only checked length and hex alphabet, so a copy-paste placeholder seed silently produced a publicly-known Ed25519 keypair (the all-zero seed yields public key `3b6a27bc...da29` — anyone using the same placeholder signs identically) and was used for every TAP request. The error message now mentions "no entropy", names the offending byte value, and points operators at `openssl rand -hex 32`. Operators currently running with such a placeholder will see a startup failure — security-positive. Regression tests in `tap-mcp-server::tests` cover all-zero, all-ones, and all-bytes-equal seeds (#148)
- **Breaking (security):** `TapVerifier`'s replay-protection cache no longer uses LRU eviction. The previous `LruCache<nonce, expires>` would silently drop the least-recently-used entry once full, even when its `expires` had not passed — letting an attacker who could flood the verifier with `capacity + 1` distinct fresh nonces inside an 8-minute window evict a legitimate captured signature and successfully replay it. The cache is now a TTL-driven `HashMap`: entries are held until `expires + CLOCK_SKEW_TOLERANCE_SECS`, which is exactly when the signature itself would also fail timestamp validation. When the cache is at capacity with only unexpired entries, fresh requests are refused fail-closed with the new `BridgeError::ReplayCacheSaturated` variant rather than evicting a still-valid nonce. `TapVerifier::new(capacity)` now documents the sizing rule (`peak_RPS × 540` to cover the validity window plus skew); under-sized deployments must increase capacity or apply upstream rate limiting. Regression tests in `tap::verifier` flood the cache with `capacity` extra distinct nonces and assert the original is still detected as a replay (#153)
- `tap-mcp-server`'s `Config::validate_agent_directory` is hardened to mirror the bridge's `parse_merchant_url`. Previously it only checked for the `https://` prefix and a successful `url::Url::parse`, so a misconfigured `TAP_AGENT_DIRECTORY` (loopback host, embedded userinfo, port 0, `..` path-traversal) would let the server start cleanly while every signed TAP request carried a `Signature-Agent` header that merchants couldn't fetch JWKS from — silently breaking signature verification on the merchant side. The validator now rejects loopback hosts (`localhost`, `127.0.0.1`, `[::1]`), URLs with userinfo (which would leak credentials into the `Signature-Agent` header), port `0`, and `.` / `..` path segments (which `url::Url::parse` silently normalises away). New unit tests in `tap-mcp-server::tests` cover each rejection case (#149)
- `MerchantConfig::validate` now closes a cluster of operator-input gaps in the four TOML validators (`validate_base_url`, `validate_oauth_token_url`, `validate_endpoint_path`, `validate_field_name`). A new shared `validate_https_url` helper rejects loopback in every spelling (`localhost`, `127.0.0.0/8`, `::1`, the bracketed `[::1]` form `host_str()` returns for IPv6 URLs, IPv4-mapped IPv6 loopback such as `::ffff:127.0.0.1`), the unspecified addresses `0.0.0.0` / `::`, URLs that embed userinfo (`https://user:pass@host` — a silent way to bypass `client_id_env`/`client_secret_env` env-var indirection), and explicit port `0`. `validate_endpoint_path` additionally rejects NUL bytes, query strings (`?`), fragments (`#`), and whitespace — endpoint templates are paths only and `build_url_with_query` appends query params separately. `validate_field_name` now rejects empty keys and matches the prototype-pollution forbidden list case-insensitively, so `__PROTO__` / `Constructor` are caught alongside `__proto__` / `constructor` (#150)
- **Breaking (security):** All three workspace HTTP clients (`mcp::http::create_http_client`, `mcp::subscriptions::tools::SUBSCRIPTION_HTTP_CLIENT`, `transport::http::DEFAULT_HTTP_CLIENT`, plus `HttpTransport::with_config`) now build with `reqwest::redirect::Policy::none()`. `reqwest`'s default `Policy::limited(10)` would forward TAP-specific headers (`Signature`, `Signature-Input`, `Signature-Agent`, `Content-Digest`) and 307/308 request bodies — which for ACRO requests are unencrypted PII (`country_code`, `zip`, `ip_address`, `user_agent`, `platform`) tied to `consumer_id` — to a redirect target on a different host. Only `Authorization`, `Cookie`, `Proxy-Authorization`, `Cookie2` are stripped by reqwest on cross-host redirects; TAP headers are not. RFC 9421 §1 binds a signature to a specific request, so silently following a 30x violates the protocol's authority guarantees. After this fix, a merchant returning 30x surfaces as a `BridgeError::MerchantError` carrying the status code rather than being followed; callers that legitimately need redirect behavior must implement it at a higher layer (re-signing for the new `@path`/`@authority`). Callers relying on the previous behavior will start receiving errors. Regression tests in `mcp::http::tests` and `transport::http::tests` assert that a 307 response is returned as-is and not followed (#144)
- **Security:** Close the redirect-leak hole that #144 missed. The `HTTP_CLIENT` `LazyLock` static in `mcp::tools` — used by `checkout_with_tap` and `browse_merchant` end-to-end — was still using `reqwest`'s default `Policy::limited(10)` and would forward signed TAP headers and unencrypted ACRO bodies to whichever host a merchant 30x'd to. The static now builds through a shared `tap_http_client_builder()` helper that pins `reqwest::redirect::Policy::none()`, matching the other four clients fixed in #144. A regression test in `mcp::tools::tests` mirrors `mcp::http::tests::test_create_http_client_does_not_follow_redirects` so any future refactor that drops the redirect policy on this code path fails CI (#146)
